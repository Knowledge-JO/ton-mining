...
export default function MinerCard() {
  ...
  async function getMinerDetailsByUserId(userId) {
    if (!userId) {  // Check if userId is undefined
    ...
    <>
      <Flex direction={"column"} p={5} bg={"#10062D"}>
        ...
        <SimpleGrid columns={{ base: 1, sm: 2, md: 2, lg: 4 }} spacing={4}>
          {minerDeets && minerDeets.map((miner)=>{
            return(
            <Card
             key={miner.minerId}
              border="2px solid #301287"
            ...
            >
              ...
              <Stack  p={5} >
                <Flex align={"start"} justify={"space-between"} alignItems={'center'}>
                  <Text>#{miner?.minerId.slice(0,7)}</Text>
                  <Menu>
  <MenuButton as={IconButton} icon={<IoMdMore />} variant='outline'  aria-label='Options' />
  <MenuList>
    <MenuItem icon={<TbTriangleSquareCircle />}>Upgrade</MenuItem>
    <MenuItem  onClick={() => handleMintClick(miner.minerId)}>Mint</MenuItem>
